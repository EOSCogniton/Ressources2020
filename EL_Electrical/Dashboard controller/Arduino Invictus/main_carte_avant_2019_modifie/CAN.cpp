...
//    Serial.print(Print);
    ...
    if(tmillis>(T_D_Millis+T_Time)){ // Envoie discret de période T_Time
        ...
        if(Switch_L==0){
            CAN0.sendMsgBuf(0x1003, 1, 8, Null);
        }
        if(Switch_L==1){
            CAN0.sendMsgBuf(0x1003, 1, 8, Modif);
        }
        if(Switch_C==0){
            CAN0.sendMsgBuf(0x1004, 1, 8, Null);
        }
        if(Switch_C==1){
            CAN0.sendMsgBuf(0x1004, 1, 8, Modif);
        }
        if(Switch_W==0){
            CAN0.sendMsgBuf(0x1005, 1, 8, Null);
        }
        if(Switch_W==1){
            CAN0.sendMsgBuf(0x1005, 1, 8, Modif);
        }
        if(Switch_G==0){
            CAN0.sendMsgBuf(0x1006, 1, 8, Null);
        }
        if(Switch_G==1){
            CAN0.sendMsgBuf(0x1006, 1, 8, Modif);
        }
    }
}
